1️⃣ Project Title
 🧠 HealthMate AI 

2️⃣ Short Description
HealthMate AI is a Machine Learning based web application that predicts multiple diseases such as Diabetes, Heart Disease, and Parkinson’s.  
It also includes an AI-powered health assistant, a doctor consultation module, and an emergency support system.

3️⃣ 🚀 Features
- Diabetes Prediction
- Heart Disease Prediction
- Parkinson’s Prediction
- AI Health Assistant (Text & Voice)
- Doctor Consultation
- Emergency Helpline System
- User-friendly Streamlit UI
- Fast and accurate results

4️⃣ 🛠️ Technologies Used
- Python
- Streamlit
- Scikit-learn
- Pandas & NumPy
- Google Gemini AI
- Machine Learning (SVM Model)
  
5️⃣ 📂 Project Structure
HealthMate-AI/
│── app.py
│── diabetes_model.sav
│── heart_disease_model.sav
│── parkinsons_model.sav
│── requirements.txt
│── README.md

6️⃣ ⚙️ Installation & Run Locally

1️⃣ Clone the repository

git clone https://github.com/your-username/HealthMate-AI.git

2️⃣ Install dependencies

pip install -r requirements.txt

7️⃣ 🔑 API Key Setup (Gemini)
Create a `.streamlit/secrets.toml` file and add:
GEMINI_API_KEY="your_api_key"

8️⃣ 🌐 Deployment
This project is deployed on Streamlit Cloud and can be accessed from anywhere.
(Deploy hone ke baad link add kar dena)

9️⃣ 🎯 Future Enhancements
- User login system
- PDF health report generation
- Real-time hospital locator
- Database integration
  
🔟 👨‍💻 Author
Anurag  

⭐ BONUS
Add this at the top:
![Python](https://img.shields.io/badge/Python-3.10-blue)
![Streamlit](https://img.shields.io/badge/Streamlit-Deployed-red)
![ML](https://img.shields.io/badge/MachineLearning-Enabled-green)

## 🚀 Live Demo
[![Live App](https://img.shields.io/badge/Click%20Here-Launch%20App-brightgreen?style=for-the-badge)](https://your-app-link.streamlit.app)
